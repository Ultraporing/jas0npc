...
def MAIN():
    ...
    addDir(40,'url','None','','','[B][COLOR yellow]Resolver Settings[/B][/COLOR]','%s/resources/art/settings.jpg'%ADDON.getAddonInfo("path"))
...
def addDir(mode,url,types,linkback,meta_name,name,iconimage):
    ...
    ok=True
    infoLabels = GRABMETA(name,types,meta_name,url)
...
if mode==None or url==None or len(url)<1:
    ...
    if search_entered == None or len(search_entered)<1:
        ...
        types = 'movie'
        GET_INDEX(url,types,meta_name)
    GA("None","MOVIE SEARCH: "+search_entered)
elif mode == 300:

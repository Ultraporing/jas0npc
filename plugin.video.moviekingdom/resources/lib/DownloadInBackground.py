...
selfAddon = xbmcaddon.Addon(id='plugin.video.moviekingdom')
addon_path = selfAddon.getAddonInfo('path')
addon_id = 'plugin.video.moviekingdom'
...
print 'uq_url: '+uq_url+' '+sys.argv[1]
print 'ug_dest: '+uq_dest+' '+sys.argv[2]
print 'uq_file: '+uq_file+' '+sys.argv[3]
print 'uq_Oname: '+uq_Oname
...
art = xbmc.translatePath(os.path.join(selfAddon.getAddonInfo('path'), 'resources', 'art'))

print 'art'
print art+'mkdownj.png'
print art
print os.path.join(art, 'mkdownj.png')
...
def Notification(currentPercent):
    if currentPercent in NotifyPercents:
        ...
        icon_file =os.path.join(art, 'mkdownj.png')
        print '        Download progress...' + str(currentPercent)+'% for file ' + uq_file
        xbmc.executebuiltin( "XBMC.Notification(%s,%s,%i,%s)" % ( 'Download Progress - ' + str(currentPercent)+'%', uq_file, 5000,os.path.join(art, 'mkdownj.png') ) )
